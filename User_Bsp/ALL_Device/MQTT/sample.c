...
_Bool OneNet_SendData(FORMAT_TYPE type, char *devid, char *apikey, DATA_STREAM *streamArray, unsigned short streamArrayCnt)
{
	...
	Uart_printf(USART_DEBUG, "Tips:	OneNet_SendData-MQTT_TYPE%d\r\n", type);
	...
	if (body_len)
	{
		...
		if (MQTT_PacketSaveData(devid, body_len, NULL, (uint8)type, &mqttPacket) == 0)
		{
			...
			if (body_len)
			{
				...
				Uart_printf(USART_DEBUG, "WARN:	DSTREAM_GetDataStream_Body Failed\r\n");

			//---------------------------------------------步骤五：删包-------------------------------------------------------
			MQTT_DeleteBuffer(&mqttPacket);
		}
		else
			Uart_printf(USART_DEBUG, "WARN:	MQTT_NewBuffer Failed\r\n");
	}
	else
		status = 1;
...
void OneNet_HeartBeat(void)
{
	...
	while (sCount--)
	{
		...
		{
			Uart_printf(USART_DEBUG, "Tips:	HeartBeat OK\r\n");

			break;
		}
		else
		{
			Uart_printf(USART_DEBUG, "Check Device\r\n");
		}
...
_Bool OneNet_UnSubscribe(const char *topics[], unsigned char topic_cnt)
{
	...
	for (; i < topic_cnt; i++)
		Uart_printf(USART_DEBUG, "UnSubscribe Topic: %s\r\n", topics[i]);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	case MQTT_PKT_CMD: // 命令下发
		...
		if (result == 0)
		{
			Uart_printf(USART_DEBUG, "cmdid: %s, req: %s\r\n", cmdid_topic, req_payload);

			if (MQTT_PacketCmdResp(cmdid_topic, req_payload, &mqttPacket) == 0) // 命令回复组包
			{
				Uart_printf(USART_DEBUG, "Tips:	Send CmdResp\r\n");
	...
	case MQTT_PKT_PUBLISH: // 接收的Publish消息
		...
		if (result == 0)
		{
			Uart_printf(USART_DEBUG, "topic: %s\r\npayload: %s\r\n", cmdid_topic, req_payload);
			...
			case 1: // 收到publish的qos为1，设备需要回复Ack
				...
				{
					Uart_printf(USART_DEBUG, "Tips:	Send PublishAck\r\n");
					NET_DEVICE_SendData(mqttPacket._data, mqttPacket._len);
					MQTT_DeleteBuffer(&mqttPacket);
				}
			...
			case 2: // 收到publish的qos为2，设备先回复Rec
				...
				{
					Uart_printf(USART_DEBUG, "Tips:	Send PublishRec\r\n");
					NET_DEVICE_SendData(mqttPacket._data, mqttPacket._len);
					MQTT_DeleteBuffer(&mqttPacket);
				}
	...
	case MQTT_PKT_PUBACK: // 发送Publish消息，平台回复的Ack
		...
		if (MQTT_UnPacketPublishAck(cmd) == 0)
			Uart_printf(USART_DEBUG, "Tips:	MQTT Publish Send OK\r\n");
	...
	case MQTT_PKT_PUBREC: // 发送Publish消息，平台回复的Rec，设备需回复Rel消息
		...
		{
			Uart_printf(USART_DEBUG, "Tips:	Rev PublishRec\r\n");
			if (MQTT_PacketPublishRel(MQTT_PUBLISH_ID, &mqttPacket) == 0)
			{
				Uart_printf(USART_DEBUG, "Tips:	Send PublishRel\r\n");
				NET_DEVICE_SendData(mqttPacket._data, mqttPacket._len);
				MQTT_DeleteBuffer(&mqttPacket);
			}
		}
	...
	case MQTT_PKT_PUBREL: // 收到Publish消息，设备回复Rec后，平台回复的Rel，设备需再回复Comp
		...
		{
			Uart_printf(USART_DEBUG, "Tips:	Rev PublishRel\r\n");
			if (MQTT_PacketPublishComp(MQTT_PUBLISH_ID, &mqttPacket) == 0)
			{
				Uart_printf(USART_DEBUG, "Tips:	Send PublishComp\r\n");
				NET_DEVICE_SendData(mqttPacket._data, mqttPacket._len);
				MQTT_DeleteBuffer(&mqttPacket);
			}
		}
	...
	case MQTT_PKT_PUBCOMP: // 发送Publish消息，平台返回Rec，设备回复Rel，平台再返回的Comp
		...
		{
			Uart_printf(USART_DEBUG, "Tips:	Rev PublishComp\r\n");
		}
	...
	case MQTT_PKT_SUBACK: // 发送Subscribe消息的Ack
		...
		if (MQTT_UnPacketSubscribe(cmd) == 0)
			Uart_printf(USART_DEBUG, "Tips:	MQTT Subscribe OK\r\n");
		else
			Uart_printf(USART_DEBUG, "Tips:	MQTT Subscribe Err\r\n");
	...
	case MQTT_PKT_UNSUBACK: // 发送UnSubscribe消息的Ack
		...
		if (MQTT_UnPacketUnSubscribe(cmd) == 0)
			Uart_printf(USART_DEBUG, "Tips:	MQTT UnSubscribe OK\r\n");
		else
			Uart_printf(USART_DEBUG, "Tips:	MQTT UnSubscribe Err\r\n");
