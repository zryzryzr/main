...
void MQTT_NewBuffer(MQTT_PACKET_STRUCTURE *mqttPacket, uint32 size)
{
	...
	{
		mqttPacket->_memFlag = MEM_FLAG_ALLOC; // 1，动态分配内存

		mqttPacket->_data = (uint8 *)MQTT_MallocBuffer(size);

		// malloc(size)：malloc 是 C 标准库中的函数，它从堆中分配一块大小为 size 字节的内存，
		// 并返回一个指向该内存块的指针。如果分配失败，它会返回 NULL。
	...
	{
		mqttPacket->_memFlag = MEM_FLAG_STATIC; // 2，静态分配内存
...
void MQTT_DeleteBuffer(MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if (mqttPacket->_memFlag == MEM_FLAG_ALLOC)
		MQTT_FreeBuffer(mqttPacket->_data); // （free）
...
/*
这个函数 MQTT_DumpLength 的功能是将一个长度值（len）按照 MQTT 协议的可变长度编码格式
（Variable Length Encoding）编码到一个缓冲区（buf）中，并返回编码使用的字节数。
MQTT 协议使用这种编码方式来表示某些字段（例如消息的总长度），以便用更少的字节表示较小的数字，
同时允许更大的数字占用多个字节。
*/
/*
例子
...
len = 300
计算 300 % 128 = 44，将 44 存入第一个字节。
计算 len >>= 7，得到 300 / 128 = 2（剩余的长度）。
因为 len > 0，设置最高位为 1，第一个字节为 44 | 128 = 172。
第二步：

len = 2
计算 2 % 128 = 2，将 2 存入第二个字节。
计算 len >>= 7，得到 2 / 128 = 0（长度处理完毕）。
因为 len == 0，设置最高位为 0，第二个字节为 2。
最终，300 被编码为两个字节：

第一个字节为 172（0b10101100），
第二个字节为 2（0b00000010）。
*/
...
/*
函数 MQTT_ReadLength 用于从输入流中读取一个使用 MQTT 协议的可变长度编码格式编码的长度值。
stream：指向输入流的指针，包含了待读取的可变长度编码数据。
size：输入流的大小（字节数）。
len：指向一个 uint32 变量的指针，用于存储解码后的长度值。
返回值：成功时返回读取的字节数，失败时返回错误码（如 -1 表示不完整，-2 表示超出范围）。

MQTT_ReadLength 函数与之前的 MQTT_DumpLength 函数正好是相反的操作。
*/
...
//				devid：设备ID
//				cTime：连接保持时间
//				clean_session：离线消息清除标志，为1的话，只是实时的之前没发完的会话会删除
//				qos：重发标志
